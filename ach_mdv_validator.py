...
class ACHMDVValidator:
    def __init__(self, config, log_manager):
        ...
        # Retail ODFI: Type-5 positions 41-50 should NOT match bank ABA number
        self.retail_odfi_violations = []

        self.problem_line_counter = 0
        self.badlines = []
        self.badfilenames = []

        self.prepend_file_path = (
            self.config.data_path if self.config.full_file_path else ""
        )
    ...
    def validate_files(self):
        try:
            ...
            fileNames = [f for f in fileNames if not any(f.lower().endswith(ext) for ext in excluded_extensions)]
            filtered_count = original_count - len(fileNames)

            if filtered_count > 0:
                self.logger.info(f"Filtered out {filtered_count} encrypted/compressed files by extension")
            ...
            if getattr(self.config, "aba_number", ""):
                self.logger.info(f"Retail ODFI check enabled (bank ABA={self.config.aba_number}); Type-5 pos 41-50 must NOT match")
    ...
    def _validate_file(self, fname):
        ...
        try:
                ...
                for file_line, line_bytes in enumerate(f, 1):
                    ...
                    try:
                        ...
                        if consecutive_errors <= 3:
                            ...
                            self.problematic_files.add(fname)
                            self.logger.error(
                                f"[Bad Key] File={fname} Line={file_line}"
                            )
                        key = 10
                    ...
                    if key == 1:
                        ...
                        if type_1_count > 1:
                            ...
                            self.problematic_files.add(fname)
                            self.problem_counter += 1
                            self.multiple_header_files.append((fname, file_line, type_1_count))
                            self.logger.error(
                                f"[Multiple Headers] File={fname} has {type_1_count} Type-1 headers (concatenated file detected) at line {file_line}"
                            )
                    ...
                    if key == 5:
                        ...
                        if (
                            sec_code not in self.config.sec_codes
                        ...
                        # Retail ODFI check: Type-5 positions 41-50 (cut -c41-50) should NOT match bank ABA
                        if getattr(self.config, "aba_number", "") and len(line) >= 50:
                            odfi_field = line[40:50]
                            aba_digits = "".join(c for c in self.config.aba_number if c.isdigit())
                            odfi_digits = "".join(c for c in odfi_field if c.isdigit())
                            # Match: ODFI ID equals bank ABA, or contains it (8-digit ODFI = first 8 of 9-digit ABA)
                            aba_8 = aba_digits[:8] if len(aba_digits) >= 8 else aba_digits
                            is_match = (
                                aba_digits
                                and odfi_digits
                                and (
                                    odfi_digits == aba_digits
                                    or odfi_digits == aba_8
                                    or aba_digits in odfi_digits
                                    or aba_8 in odfi_digits
                                )
                            )
                            if is_match:
                                self.retail_odfi_violations.append((fname, file_line, odfi_field.strip()))
                                self.problematic_files.add(fname)
                                self.problem_counter += 1
                                self.logger.error(
                                    f"[Retail ODFI] File={fname} Line={file_line}: Type-5 ODFI ID '{odfi_field.strip()}' matches bank ABA '{self.config.aba_number}'"
                                )
                    ...
                    if key == 8:
                        if sec_code in ["IAT", "POS"]:
                            if not self.test_seven_record_test(
                                sec_code, seven_record_list
                            ):
                                error_key = (fname, sec_code)
                                if error_key not in self.bad_7_addendum[sec_code]:
                                    self.bad_7_addendum[sec_code][error_key] = {
                                        'missing': sorted(set(map(str, range(10, 17))) - set(seven_record_list)) if sec_code == "IAT" else [],
                    ...
                    self._process_problem_line(
                        key, row_length, line, fname, file_line
                    )
            ...
            if self.last_key_seen != 9:
                ...
                self.logger.error(
                    f"[EOF Error] File did not end with key=9: {fname}"
                )

        except Exception as e:
            self.logger.error(
                f"Unhandled error validating file {fname}: {e}"
            )
            self.logger.debug(traceback.format_exc())
    ...
    def summarize_results(self, total_files):
        ...
        if len(self.skipped_binary_files) > 0:
            self.logger.info("")
            self.logger.info(f"Skipped {len(self.skipped_binary_files)} binary/encrypted files:")
            for fname in self.skipped_binary_files[:10]:
                self.logger.info(f"  - {fname}")
            if len(self.skipped_binary_files) > 10:
                self.logger.info(f"  ... and {len(self.skipped_binary_files) - 10} more")
            self.logger.info("")

        checks = [
            ("Bad Keys", len(self.bad_keys), self.bad_keys),
            ("Bad SEC Codes", len(self.bad_secs), self.bad_secs),
            ("Missing File Header (Type 1)", self.no_one_record, self.missing_header_files),
            ("Multiple Headers (Concatenated Files)", len(self.multiple_header_files), self.multiple_header_files),
            ("Record Order Issues", sum(len(v) for v in self.bad_order.values()), self.bad_order),
            ("Bad Record Lengths", sum(len(v) for v in self.bad_lengths.values()), self.bad_lengths),
        ...
        if getattr(self.config, "aba_number", ""):
            checks.insert(
                -1,
                ("Retail ODFI (Type-5 ODFI ID matches bank ABA)", len(self.retail_odfi_violations), self.retail_odfi_violations),
            )

        if self.iat_bad_addendum_files > 0:
            self.logger.info("")
            self.logger.info("IAT Addendum Error Details (showing up to 10 unique errors):")
            for idx, (error_key, details) in enumerate(list(self.bad_7_addendum["IAT"].items())[:10], 1):
                fname, sec_code = error_key
                self.logger.error(
                    f"  {idx}. File={fname}, Missing={details['missing']}, Found={details['found']}"
                )
            if len(self.bad_7_addendum["IAT"]) > 10:
                self.logger.info(f"  ... and {len(self.bad_7_addendum['IAT']) - 10} more files with IAT addendum errors")

        if self.pos_bad_addendum_files > 0:
            self.logger.info("")
            self.logger.info("POS Addendum Error Details (showing up to 10 unique errors):")
            for idx, (error_key, details) in enumerate(list(self.bad_7_addendum["POS"].items())[:10], 1):
                fname, sec_code = error_key
            ...
            if len(self.bad_7_addendum["POS"]) > 10:
                self.logger.info(f"  ... and {len(self.bad_7_addendum['POS']) - 10} more files with POS addendum errors")
        ...
        self.logger.info(f"  Skipped binary/encrypted files: {len(self.skipped_binary_files)}")
        self.logger.info("=" * 40)

        error_percent = round(
            100 * len(self.problematic_files) / max(1, total_files), 2
        )
    ...
    def _print_error_details(self, check_name, error_data):
        ...
        if check_name == "Bad Keys":
            ...
            if len(error_data) > 10:
                self.logger.info(f"    ... and {len(error_data) - 10} more bad key errors")
            ...
            for idx, (fname, line, sec_code, file_line) in enumerate(error_data[:10], 1):
                self.logger.error(f"    {idx}. File={fname}, Line={file_line}, SEC={sec_code}")
            if len(error_data) > 10:
                self.logger.info(f"    ... and {len(error_data) - 10} more bad SEC code errors")
            ...
            if len(error_data) > 10:
                self.logger.info(f"    ... and {len(error_data) - 10} more files missing header")
            ...
            for idx, (fname, file_line, header_count) in enumerate(error_data[:10], 1):
                self.logger.error(f"    {idx}. File={fname}, Total Headers={header_count}, Last at Line={file_line}")
            if len(error_data) > 10:
                self.logger.info(f"    ... and {len(error_data) - 10} more files with multiple headers")
            ...
            if total_order_errors > 10:
                self.logger.info(f"    ... and {total_order_errors - count} more order errors")
            ...
            if total_length_errors > 10:
                self.logger.info(f"    ... and {total_length_errors - count} more length errors")
            ...
            if len(error_data) > 10:
                self.logger.info(f"    ... and {len(error_data) - 10} more IAT addendum errors")
            ...
            if len(error_data) > 10:
                self.logger.info(f"    ... and {len(error_data) - 10} more POS addendum errors")

        elif check_name == "Retail ODFI (Type-5 ODFI ID matches bank ABA)":
            self.logger.info("  Sample Retail ODFI Violations (up to 10):")
            for idx, (fname, file_line, odfi_field) in enumerate(error_data[:10], 1):
                self.logger.error(f"    {idx}. File={fname}, Line={file_line}, ODFI ID (pos 41-50)='{odfi_field}'")
            if len(error_data) > 10:
                self.logger.info(f"    ... and {len(error_data) - 10} more Retail ODFI violations")
            ...
            if len(error_data) > 10:
                self.logger.info(f"    ... and {len(error_data) - 10} more files missing EOF")
