...
def validate_file_extensions(data_path, logger=None):
    ...
    try:
        ...
        if invalid_files:
            error_msg = "Please ensure the extension is correct, only .ACH is acceptable. It is case sensitive."
...
def main():
    ...
    logfile = f"{tenant_name}_{config.sid}_{datetime.now().strftime('%Y%m%d_%H%M')}.ACH.log"

    log_manager, full_log_path = LogManager.setup(config.log_path, logfile, config.log_level)
    ...
    logger.info(f"File extension validation: PASSED (all files have .ACH extension)")
    ...
    try:
        ...
        if not ach_type:
            ...
            logger.warning(f"ABA analyzer returned no type; falling back to config value: {ach_type}")
    except Exception as e:
