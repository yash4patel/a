...
class Config:
    ...
    def _load(self):
        ...
        if self.section not in config:
            raise ValueError(f"Section '{self.section}' not found in config file {self.filepath}")

        conf = config[self.section]

        # required fields (use get with defaults where acceptable)
        self.tenant_name = conf.get("tenant_name", "").strip()
        self.sid = conf.get("sid", "").strip()
        self.data_path = folder_tools.check_directory_format(conf.get("data_path", ""), silent=True)
        self.log_path = folder_tools.check_directory_format(conf.get("log_path", ""), silent=True)
        self.sec_codes = [code.strip() for code in conf.get("sec_codes", "").split(",") if code.strip()]
        ...
        self.problem_line_types = None if problem_line_types.lower() == "none" else list(map(int, problem_line_types.split(",")))
        self.extension = conf.get("extension", "ACH")
        self.ach_type = conf.get("ach_type", "")
        self.is_folded = conf.getboolean("is_folded", True)
        # Bank ABA number for Retail ODFI check (positions 41-50 of Type-5; should NOT match)
