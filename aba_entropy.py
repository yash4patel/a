...
class ABAEntropyAnalyzer:
                    ...
                    with open(filepath, "rb") as f:
                        for line_bytes in f:
                            ...
                            if key == 5:
                                try:
                                    odfi = line[81:87]
                                    self.aba_5_counts[odfi] = self.aba_5_counts.get(odfi, 0) + 1
                                except Exception:
                                    continue
                            elif key == 6:
                                try:
                                    rdfi = line[5:12]
                                    self.aba_6_counts[rdfi] = self.aba_6_counts.get(rdfi, 0) + 1
                                except Exception:
                                    continue
    ...
    def _report_results(self):
        try:
            ...
            aba_5_sum = sum(aba_5)
            ent_aba_5 = sum([math.log2(i / aba_5_sum) * i / aba_5_sum for i in aba_5]) if aba_5_sum > 0 else 0

            aba_6 = list(self.aba_6_counts.values())
            aba_6_sum = sum(aba_6)
            ent_aba_6 = sum([math.log2(i / aba_6_sum) * i / aba_6_sum for i in aba_6]) if aba_6_sum > 0 else 0
            ...
            self.logger.info(f"ABA counts (Type-5): {len(self.aba_5_counts)} distinct, total {aba_5_sum}")
            self.logger.info(f"ABA counts (Type-6): {len(self.aba_6_counts)} distinct, total {aba_6_sum}")
            self.logger.info("")
